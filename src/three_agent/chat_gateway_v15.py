...
class ContextAwareProjectChatService(IntentAwareProjectChatService):
    """Direct chat with deterministic, reference-gated conversation context."""
    ...
    def _language_for_follow_up(
        ...
        *,
        ...
        try:
            ...
            # The inherited owner-scoped ensure_conversation path remains the
            # authority for invalid/unavailable conversation IDs.
            return language
    ...
    def _recent_context(self, job, *, max_chars: int = DEFAULT_CONTEXT_MAX_CHARS) -> str:
        # Keep this method for compatibility with v14 callers/tests, but context
        # eligibility is now decided by the deterministic plan.
        plan = self._context_plan(job)
        if max_chars < DEFAULT_CONTEXT_MAX_CHARS and plan.text:
            # Rebuild with the requested smaller budget rather than slicing role
            # boundaries from an already-rendered context block.
            with self._lock:
                conversation_id = self._job_conversations.get(job.job_id)
            if conversation_id:
                try:
...
class ContextAwareWorkflowDispatchHTTPHandler(IntentAwareWorkflowDispatchHTTPHandler):
    """V14 HTTP/auth/workflow behavior plus deterministic chat-context policy."""
...
def main() -> int:
    ...
    httpd = ThreadingHTTPServer((host, port), ContextAwareWorkflowDispatchHTTPHandler)
    ...
    print("[WorkSpace] Conversation history is injected only for explicit follow-up/reference requests.", flush=True)

...
<main>
  ...
  <section id="overview" class="panel active">
    ...
    <section class="card" style="margin-top:14px">
      ...
      <p class="muted">Risk・Asset Health・Correlation・Flow・Timeline を固定集計値で統合します。</p>
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _json(self, status: int, payload: dict[str, object]) -> None:
        ...
        self.send_response(status); self._headers("application/json; charset=utf-8"); self.send_header("Content-Length",str(len(raw))); self.end_headers(); self.wfile.write(raw)

    def _html(self) -> None:
        page=_PAGE.replace("__CSRF_TOKEN__",self.server.csrf_token).replace("__CSP_NONCE__",self.server.csp_nonce).replace("__DEMO_MODE__","true" if self.server.demo_mode else "false")
        raw=page.encode("utf-8"); self.send_response(200); self._headers("text/html; charset=utf-8"); self.send_header("Content-Length",str(len(raw))); self.end_headers(); self.wfile.write(raw)

    def _host_ok(self) -> bool:
        if _request_host_is_loopback(self.headers.get("Host")):
            return True
        self._json(421,{"status":"blocked","reason_code":"LOOPBACK_HOST_REQUIRED"}); return False

    def _body(self) -> dict[str, object] | None:
        try: length=int(self.headers.get("Content-Length") or "0")
        except ValueError:
            self._json(400,{"status":"blocked","reason_code":"INVALID_CONTENT_LENGTH"}); return None
        if length <= 0 or length > MAX_REQUEST_BYTES:
            self._json(400,{"status":"blocked","reason_code":"INVALID_REQUEST_SIZE"}); return None
        try: payload=json.loads(self.rfile.read(length).decode("utf-8"))
        except (UnicodeDecodeError,json.JSONDecodeError):
            self._json(400,{"status":"blocked","reason_code":"INVALID_JSON"}); return None
        if not isinstance(payload,dict):
            self._json(400,{"status":"blocked","reason_code":"JSON_OBJECT_REQUIRED"}); return None
        return payload

    def _csrf_ok(self) -> bool:
        supplied=str(self.headers.get("X-Workspace-CSRF") or "")
        if supplied and hmac.compare_digest(supplied,self.server.csrf_token): return True
        self._json(403,{"status":"blocked","reason_code":"CSRF_TOKEN_REQUIRED"}); return False

    def do_GET(self) -> None:
        if not self._host_ok(): return
        path=urlsplit(self.path).path
        try:
            if path == "/": self._html(); return
            if path == "/api/v1/health":
                self._json(200,{"status":"ok","service":"WorkSpace Security Console","local_only":True,"write_authority":False,"bounded_local_initialize":True,"demo_mode":self.server.demo_mode}); return
            routes = {
                "/api/v1/security/monitoring/summary": self.server.service.summary,
                "/api/v1/security/monitoring/readiness": self.server.service.readiness,
                "/api/v1/security/monitoring/asset-intelligence": self.server.service.asset_intelligence,
                "/api/v1/security/monitoring/evidence-summary": self.server.service.evidence_summary,
                "/api/v1/security/monitoring/incident-posture": self.server.service.incident_posture,
                "/api/v1/security/monitoring/operator-posture": self.server.service.operator_posture,
            }
            if path in routes:
                self._json(200,routes[path]()); return
            if path == "/api/v1/security/monitoring/analyst-snapshot":
                self._json(200,safe_analyst_snapshot(self.server.service.load_config())); return
    ...
    def do_POST(self) -> None:
        if not self._host_ok() or not self._csrf_ok(): return
        path=urlsplit(self.path).path
        if path not in {"/api/v1/security/monitoring/run-hourly","/api/v1/security/monitoring/initialize"}:
            self._json(404,{"status":"not_found","reason_code":"ENDPOINT_NOT_FOUND"}); return
        payload=self._body()
        if payload is None: return
        if path.endswith("/run-hourly"):
            if set(payload) != {"confirm_readonly"}:
                self._json(400,{"status":"blocked","reason_code":"UNSUPPORTED_REQUEST_FIELDS"}); return
            if payload.get("confirm_readonly") is not True:
                self._json(409,{"status":"blocked","reason_code":"USER_CONFIRMATION_REQUIRED"}); return
            if self.server.demo_mode:
                self._json(409,{"status":"blocked","reason_code":"DEMO_MODE_NETWORK_EXECUTION_DISABLED"}); return
            try:
                result=self.server.service.run_hourly(execute_readonly=True)
            except MonitoringRunAlreadyLocked:
                self._json(409,{"status":"blocked","reason_code":"MONITORING_RUN_ALREADY_LOCKED"}); return
            except RuntimeError as exc:
                self._json(409,{"status":"blocked","reason_code":str(exc)[:256]}); return
...
def build_server(host: str, port: int, service: SecurityMonitoringService, *, csrf_token: str | None = None, csp_nonce: str | None = None, demo_mode: bool = False) -> _Server:
    bind=validate_loopback_host(host)
    return _Server((bind,int(port)),service,csrf_token=csrf_token or secrets.token_hex(32),csp_nonce=csp_nonce or secrets.token_hex(16),demo_mode=demo_mode)
...
def main(argv: list[str] | None = None) -> int:
    parser=argparse.ArgumentParser(description="WorkSpace local-only security monitoring console")
    parser.add_argument("--config",type=Path,default=None,help="Monitoring config path. Defaults to the portable WorkSpace user config.")
    parser.add_argument("--host",default="127.0.0.1")
    parser.add_argument("--port",type=int,default=8765)
    parser.add_argument("--demo",action="store_true",help="Launch isolated synthetic demo data with all real-network execution disabled.")
    ...
    if args.demo: print("DEMO MODE: synthetic data only; real network execution is disabled")

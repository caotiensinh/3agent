...
from .capability_revocation import CapabilityRevocation
from .execution_budget import ExecutionBudgetExceeded
from .execution_observation import ExecutionObservation
from .execution_plan import ExecutionPlan
from .runtime_checkpoint import RuntimeCheckpoint, RuntimeCheckpointError, RuntimeRecovery, RuntimeRecoveryDecision
from .task_context import TaskContext

RUNTIME_DISPATCH_RECOVERY_BINDING_SCHEMA = "workspace-runtime-dispatch-recovery-binding/v1"
RUNTIME_DISPATCH_RECOVERY_BINDING_ENVELOPE_SCHEMA = "workspace-runtime-dispatch-recovery-binding-envelope/v1"
...
def _canonical_json(payload: Any) -> str:
    try:
        ...
        raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_NOT_CANONICAL_JSON") from exc
...
def _timestamp(value: Any, field_name: str) -> str:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise RuntimeDispatchRecoveryError(f"{field_name.upper()}_MUST_BE_TIMEZONE_AWARE")
    return value
...
class RuntimeBudgetSnapshot:
    ...
    def validate(self) -> "RuntimeBudgetSnapshot":
        if self.schema_version != BUDGET_STATE_SCHEMA:
            raise RuntimeDispatchRecoveryError("BUDGET_SNAPSHOT_SCHEMA_VERSION_MISMATCH")
        _compact(self.task_id, "budget_task_id")
        for value, field_name in (
            (self.max_steps, "max_steps"),
            (self.max_tool_calls, "max_tool_calls"),
            (self.max_model_retries, "max_model_retries"),
            (self.max_model_escalations, "max_model_escalations"),
        ...
        for value, maximum, field_name in (
            ...
            (self.model_escalations_used, self.max_model_escalations, "model_escalations_used"),
        ):
            used = _nonnegative_int(value, field_name)
            if used > maximum:
                raise RuntimeDispatchRecoveryError(f"{field_name.upper()}_EXCEEDS_LIMIT")
...
class RuntimeRevocationSnapshot:
    ...
    @classmethod
    def from_revocation(cls, value: CapabilityRevocation | Mapping[str, Any]) -> "RuntimeRevocationSnapshot":
        ...
        allowed = {"schema_version", "task_id", "capability", "reason_code", "revoked_at"}
        if set(value) not in ({"task_id", "capability", "reason_code", "revoked_at"}, allowed):
...
class RuntimeDispatchRecoveryBinding:
    ...
    def validate(self) -> "RuntimeDispatchRecoveryBinding":
        if self.schema_version != RUNTIME_DISPATCH_RECOVERY_BINDING_SCHEMA:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_SCHEMA_VERSION_MISMATCH")
        _compact(self.binding_id, "binding_id")
        _compact(self.task_id, "task_id")
        for value, field_name in (
            (self.runtime_checkpoint_fingerprint, "runtime_checkpoint_fingerprint"),
        ...
        if isinstance(self.cancelled, bool) is False:
        ...
        if self.cancelled:
            _compact(self.cancellation_reason, "cancellation_reason")
        ...
        if self.in_flight_ticket_fingerprints != _normalized_hashes(
            ...
            raise RuntimeDispatchRecoveryError("IN_FLIGHT_TICKET_FINGERPRINTS_NOT_NORMALIZED")
        if self.observation_fingerprints != _normalized_hashes(
            self.observation_fingerprints,
            "observation_fingerprint",
        ):
            raise RuntimeDispatchRecoveryError("OBSERVATION_FINGERPRINTS_NOT_NORMALIZED")
        ...
        if self.binding_id != expected_id:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_IDENTITY_MISMATCH")
        return self
    ...
    def _identity_dict(self) -> dict[str, Any]:
        return {
            ...
            "in_flight_ticket_fingerprints": list(self.in_flight_ticket_fingerprints),
    ...
    def build(
        *,
        ...
        try:
            ...
            raise RuntimeDispatchRecoveryError("RUNTIME_CHECKPOINT_REVALIDATION_FAILED") from exc
        ...
        if scheduler_snapshot["schema_version"] != EXECUTION_SCHEDULER_SCHEMA:
            raise RuntimeDispatchRecoveryError("SCHEDULER_SNAPSHOT_SCHEMA_VERSION_MISMATCH")
        if scheduler_snapshot["task_id"] != checkpoint.task_id:
            raise RuntimeDispatchRecoveryError("SCHEDULER_TASK_MISMATCH")
        if scheduler_snapshot["task_context_fingerprint"] != checkpoint.task_context_fingerprint:
        ...
        if scheduler_snapshot["parent_authority_fingerprint"] != checkpoint.parent_authority_fingerprint:
            raise RuntimeDispatchRecoveryError("SCHEDULER_AUTHORITY_MISMATCH")

        approvals = _normalized_ids(scheduler_snapshot["approved_node_ids"], "approved_node_id")
        observations = _normalized_hashes(scheduler_snapshot["observations"], "observation_fingerprint")
        in_flight = _normalized_hashes(scheduler_snapshot["in_flight"], "in_flight_ticket_fingerprint")
        ...
        if observations != checkpoint.observation_fingerprints:
            raise RuntimeDispatchRecoveryError("SCHEDULER_OBSERVATION_STATE_MISMATCH")
        ...
        normalized_revocations = tuple(
            sorted(
                (RuntimeRevocationSnapshot.from_revocation(row) for row in revocations),
                key=lambda row: row.capability,
            )
        )
        if len({row.capability for row in normalized_revocations}) != len(normalized_revocations):
            raise RuntimeDispatchRecoveryError("DUPLICATE_REVOKED_CAPABILITY")
        ...
        scheduler_identity = {
            ...
            "max_concurrency": _positive_int(scheduler_snapshot["max_concurrency"], "max_concurrency"),
            "cancelled": scheduler_snapshot["cancelled"],
            "cancellation_reason": scheduler_snapshot["cancellation_reason"],
            "dispatch_sequence": _nonnegative_int(scheduler_snapshot["dispatch_sequence"], "dispatch_sequence"),
            "approved_node_ids": list(approvals),
            "in_flight": list(in_flight),
            "observations": list(observations),
        }
        if not isinstance(scheduler_identity["cancelled"], bool):
            raise RuntimeDispatchRecoveryError("INVALID_CANCELLED_FLAG")
        if scheduler_identity["cancelled"]:
            _compact(scheduler_identity["cancellation_reason"], "cancellation_reason")
        elif scheduler_identity["cancellation_reason"] is not None:
            raise RuntimeDispatchRecoveryError("UNEXPECTED_CANCELLATION_REASON")
        ...
        identity = {
            ...
            "max_concurrency": scheduler_identity["max_concurrency"],
            "cancelled": scheduler_identity["cancelled"],
            "cancellation_reason": scheduler_identity["cancellation_reason"],
            "dispatch_sequence": scheduler_identity["dispatch_sequence"],
            "approved_node_ids": list(approvals),
            "in_flight_ticket_fingerprints": list(in_flight),
            "observation_fingerprints": list(observations),
            "budget": budget.canonical_dict(),
            "revocations": [row.canonical_dict() for row in normalized_revocations],
            "captured_at": checkpoint.captured_at,
        }
        binding = RuntimeDispatchRecoveryBinding(
            binding_id="dispatch-recovery:" + _digest(identity).split(":", 1)[1][:24],
            runtime_checkpoint_fingerprint=checkpoint.fingerprint,
            task_id=checkpoint.task_id,
            task_context_fingerprint=checkpoint.task_context_fingerprint,
            plan_fingerprint=checkpoint.plan_fingerprint,
            ...
            cancellation_reason=identity["cancellation_reason"],
...
class RuntimeDispatchRecoveryBindingCodec:
    ...
    def loads(payload: str) -> RuntimeDispatchRecoveryBinding:
        if not isinstance(payload, str) or not payload:
            raise RuntimeDispatchRecoveryError("INVALID_DISPATCH_RECOVERY_BINDING_PAYLOAD")
        try:
            envelope = json.loads(payload)
        except json.JSONDecodeError as exc:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_JSON_INVALID") from exc
        if not isinstance(envelope, dict) or set(envelope) != {
            "schema_version",
            "binding",
            "binding_fingerprint",
        }:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_ENVELOPE_SHAPE_INVALID")
        if envelope["schema_version"] != RUNTIME_DISPATCH_RECOVERY_BINDING_ENVELOPE_SCHEMA:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_ENVELOPE_SCHEMA_MISMATCH")
        raw = envelope["binding"]
        if not isinstance(raw, dict):
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_RECORD_NOT_OBJECT")
        expected = {
            "binding_id",
            "schema_version",
        ...
        if set(raw) != expected or not isinstance(raw["budget"], dict) or not isinstance(raw["revocations"], list):
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_RECORD_SHAPE_INVALID")
        binding = RuntimeDispatchRecoveryBinding(
            binding_id=raw["binding_id"],
            runtime_checkpoint_fingerprint=raw["runtime_checkpoint_fingerprint"],
            task_id=raw["task_id"],
            ...
            scheduler_state_fingerprint=raw["scheduler_state_fingerprint"],
            ...
            in_flight_ticket_fingerprints=tuple(raw["in_flight_ticket_fingerprints"]),
            observation_fingerprints=tuple(raw["observation_fingerprints"]),
            budget=RuntimeBudgetSnapshot.from_mapping(raw["budget"]),
            revocations=tuple(RuntimeRevocationSnapshot.from_revocation(row) for row in raw["revocations"]),
            captured_at=raw["captured_at"],
            schema_version=raw["schema_version"],
        ).validate()
        if envelope["binding_fingerprint"] != binding.fingerprint:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_INTEGRITY_FAILED")
        if _canonical_json(envelope) != payload:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_BINDING_ENVELOPE_NOT_CANONICAL")
        return binding
...
class RuntimeDispatchRecoveryDecision:
    ...
    def canonical_dict(self) -> dict[str, Any]:
        if self.schema_version != RUNTIME_DISPATCH_RECOVERY_DECISION_SCHEMA:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_DECISION_SCHEMA_VERSION_MISMATCH")
        if self.status not in _RECOVERY_STATUSES:
            raise RuntimeDispatchRecoveryError("INVALID_DISPATCH_RECOVERY_STATUS")
        if not isinstance(self.reason_code, str) or not _REASON_RE.fullmatch(self.reason_code):
            raise RuntimeDispatchRecoveryError("INVALID_DISPATCH_RECOVERY_REASON_CODE")
        if self.manual_dispatch_fingerprints != _normalized_hashes(
            self.manual_dispatch_fingerprints,
            "manual_dispatch_fingerprint",
        ):
            raise RuntimeDispatchRecoveryError("MANUAL_DISPATCH_FINGERPRINTS_NOT_NORMALIZED")
        if self.newly_revoked_capabilities != _normalized_ids(
            self.newly_revoked_capabilities,
            "newly_revoked_capability",
        ):
            raise RuntimeDispatchRecoveryError("NEWLY_REVOKED_CAPABILITIES_NOT_NORMALIZED")
        ...
        return {
            ...
            "manual_dispatch_fingerprints": list(self.manual_dispatch_fingerprints),
            "newly_revoked_capabilities": list(self.newly_revoked_capabilities),
    ...
    def evaluate(
        *,
        ...
        if binding.runtime_checkpoint_fingerprint != checkpoint.fingerprint:
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_CHECKPOINT_MISMATCH")
        if (
            binding.task_id != checkpoint.task_id
            or binding.task_context_fingerprint != checkpoint.task_context_fingerprint
            or binding.plan_fingerprint != checkpoint.plan_fingerprint
            or binding.parent_authority_fingerprint != checkpoint.parent_authority_fingerprint
        ):
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_CANONICAL_BINDING_MISMATCH")
        ...
        try:
            ...
            raise RuntimeDispatchRecoveryError("DISPATCH_RECOVERY_CHECKPOINT_REVALIDATION_FAILED") from exc

        try:
            live_budget = RuntimeBudgetSnapshot.from_mapping(budget_guard.snapshot())
        except (KeyError, ValueError, RuntimeError) as exc:
            raise RuntimeDispatchRecoveryError("LIVE_BUDGET_SNAPSHOT_FAILED") from exc
        if live_budget.immutable_tuple != binding.budget.immutable_tuple:
            raise RuntimeDispatchRecoveryError("LIVE_BUDGET_IMMUTABLE_STATE_MISMATCH")
        if any(live < captured for live, captured in zip(live_budget.counters, binding.budget.counters)):
            raise RuntimeDispatchRecoveryError("LIVE_BUDGET_COUNTER_ROLLBACK_DETECTED")

        try:
            live_revocations = tuple(
                sorted(
                    (RuntimeRevocationSnapshot.from_revocation(row) for row in revocation_guard.list_for_task(checkpoint.task_id)),
                    key=lambda row: row.capability,
                )
            )
        except (KeyError, ValueError, RuntimeError) as exc:
            raise RuntimeDispatchRecoveryError("LIVE_REVOCATION_SNAPSHOT_FAILED") from exc
        captured_by_capability = {row.capability: row for row in binding.revocations}
        live_by_capability = {row.capability: row for row in live_revocations}
        if len(live_by_capability) != len(live_revocations):
            raise RuntimeDispatchRecoveryError("DUPLICATE_LIVE_REVOKED_CAPABILITY")
        for capability, captured in captured_by_capability.items():
            ...
            if live is None or live != captured:
                raise RuntimeDispatchRecoveryError("LIVE_REVOCATION_ROLLBACK_DETECTED")
        newly_revoked = tuple(sorted(set(live_by_capability) - set(captured_by_capability)))

        if base.status == "COMPLETE":
            status, reason = "COMPLETE", "DISPATCH_RECOVERY_COMPLETE"
        elif base.status == "MANUAL_RECONCILIATION":
            status, reason = "MANUAL_RECONCILIATION", "CHECKPOINT_REQUIRES_MANUAL_RECONCILIATION"
        elif binding.in_flight_ticket_fingerprints:
            status, reason = "MANUAL_RECONCILIATION", "IN_FLIGHT_DISPATCH_REQUIRES_MANUAL_RECONCILIATION"
            ...
            try:
                ...
                raise RuntimeDispatchRecoveryError("LIVE_BUDGET_REVALIDATION_FAILED") from exc
            else:
                status = "RECOVERABLE"
                reason = (
        ...
        decision = RuntimeDispatchRecoveryDecision(
            ...
            manual_dispatch_fingerprints=binding.in_flight_ticket_fingerprints if status == "MANUAL_RECONCILIATION" else (),

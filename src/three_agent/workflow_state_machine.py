...
WORKFLOW_V3_MAX_WALL_TIME_MS = 86_400_000  # 24h including human wait; never unbounded.
...
class WorkflowStateMachineController:
    """Durable V3 graph runner with deterministic branch and approval semantics.

    The graph can select between already-authorized WorkSpace capabilities, but it
    cannot create new tools, shell commands, network authority, credentials, or
    model authority. Approval checkpoints pause persistently and resume only from
    the exact fingerprinted node/state revision.
    """
    ...
    def _admit(self, raw_contract: Any) -> tuple[dict[str, Any], str]:
        ...
        if sum(1 for node in nodes if node["action"] == "presentation") > 1:
            raise WorkflowStateError("V3 permits at most one presentation action")
        if sum(1 for node in nodes if node["action"] == "daily_report") > 1:
            raise WorkflowStateError("V3 permits at most one daily_report action")
        ...
        for node in nodes:
            ...
            if expected is not None:
                ...
                for child_id in branch:
                    condition = self._normalized_condition(by_id[child_id])
        ...
        # Every presentation path must come after research; every daily report must
        # come after presentation. We walk the tree because V3 intentionally forbids joins.
        stack: list[tuple[str, frozenset[str]]] = [(roots[0], frozenset())]
        visited: set[tuple[str, frozenset[str]]] = set()
        while stack:
            node_id, ancestors = stack.pop()
            marker = (node_id, ancestors)
    ...
    def _advance(
        ...
        while state.get("current_node"):
            ...
            if node["kind"] == "approval":
                return self._pause_for_approval(state, contract, node)
            ...
            if action == "output":
                return self._finish_output(state, node_id, contract)

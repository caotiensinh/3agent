...
def analyze_flow_evidence(events: Iterable[CorrelationEvent]) -> FlowEvidenceAnalysis:
    """Analyze normalized flow evidence without collection or model authority."""
    ...
    entity_refs = tuple(sorted({reference.entity_ref for item in ordered for reference in item.context.references}))
    ...
    severity_counts = tuple(
        sorted(
            (severity, sum(item.event.severity == severity for item in ordered))
            for severity in SEVERITIES
            if any(item.event.severity == severity for item in ordered)
        )
    )

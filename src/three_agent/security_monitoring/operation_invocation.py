...
class SecurityInvocationResult:
    receipt: SecurityInvocationReceipt
    output: Any


class SecurityOperationInvoker:
    """Typed, fail-closed invocation gate for v0.4 reviewed handler bindings.
    ...
    def _receipt(
        ...
        *,
        ...
        return SecurityInvocationReceipt(invocation_id=invocation_id, **{k: v for k, v in payload.items() if k != "schema_version"}).validate()

...
_ALLOWED_ASSET_FIELDS = frozenset(_ASSET_KEYS)
...
class _ClosingSQLiteConnection(sqlite3.Connection):
    """SQLite connection whose context-manager scope also releases the file handle."""
...
class SecurityMonitoringAssetOnboarding:
    ...
    host, resolves a credential, starts a collector, captures packets, opens a network
    connection, restarts a service, or performs remediation. Every write is delegated
    to the hardened canonical configuration manager so strong-confirmation and
    metadata-only audit semantics remain authoritative.
    """

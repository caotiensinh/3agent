...
class SecurityMonitoringConfigApplication(_BASE_APPLICATION):
    """Security monitoring read model plus admin-only configuration boundary."""
...
class SecurityMonitoringConfigHTTPHandler(_BASE_HANDLER):
    ...
    def _security_config_get(self) -> None:
        if self._require_admin() is None:
            return
        try:
            self._json(HTTPStatus.OK, self.app.security_config.get())
    ...
    def _security_config_post(self, action: str) -> None:
        ...
        try:
            ...
            if action == "validate":
                ...
                result = self.app.security_config.save(payload.get("config"))
            ...
            # Contract messages identify the rejected field without echoing the
            # submitted payload or any potential secret value.
    ...
    def do_GET(self) -> None:
        if urlparse(self.path).path == "/api/security/config":
...
_v17.HTML_V17 = WORKSPACE_HTML_SECURITY_V3

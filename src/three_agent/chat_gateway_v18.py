...
class SecurityMonitoringApplication(_BASE_WORKFLOW_V4_APPLICATION):
    """Current WorkSpace runtime with query-only monitoring and bounded PCAP approval."""
...
class SecurityMonitoringHTTPHandler(_BASE_WORKFLOW_V4_HANDLER):
    """Read-only Security Analyst plus admin approval metadata; never capture execution."""
    ...
    def _security_pcap_approve(self) -> None:
        ...
        if config is None:
            self._json(
                HTTPStatus.CONFLICT,
                {
                    "error": "Monitoring configuration unavailable",
                    "code": "PCAP_MONITORING_CONFIG_UNAVAILABLE",
                },
            )
            return
        if policy is None:
            self._json(
                HTTPStatus.CONFLICT,
                {
                    "error": "Incident capture configuration unavailable",
                    "code": "PCAP_CONFIGURATION_ERROR",
                },
            )
            return
...
_v17.HTML_V17 = WORKSPACE_HTML_V14

...
This module does not introduce a second adaptive-learning store or promotion
mechanism. ``KnowledgeCandidate(kind="skill")`` remains canonical. The classes
below add a skill-specific view, deterministic production preview, security
receipt, and a staging-only service over the existing verified reflection path.
...
from .skills import MAX_SKILL_BYTES, _scan_instruction_text, _skill_digest
...
def _safe_display_text(value: str, *, limit: int) -> str:
    ...
    # Frontmatter routing metadata is deliberately a compact display projection,
    # not another authority-bearing copy of arbitrary candidate syntax.
...
def _render_document(*, name: str, title: str, scope: str, content: str) -> str:
    ...
    scope_text = " ".join(str(scope or "").split()).strip()
    body = str(content or "").replace("\r\n", "\n").replace("\r", "\n").strip()
    if not title_text or not scope_text or not body:
...
def _preview_reflection_result(result: ReflectionResult) -> bool:
    """Return True only when a model result is a safe, compact skill proposal.

    This runs before ``ReflectionCoordinator`` constructs or stages a candidate.
    A 64-character placeholder name gives a conservative production-size bound;
    the real deterministic name is never longer.
    """

    result.validate()
    if result.result != "CANDIDATE" or result.kind != "skill":
        return False
    try:
...
class CandidateSkill:
    ...
    def validate(self) -> "CandidateSkill":
        ...
        if not _SHA_RE.fullmatch(self.candidate_sha256):
            raise CandidateSkillError("CANDIDATE_SKILL_SHA_INVALID")
        if not _SHA_RE.fullmatch(self.proposed_skill_sha256):
            raise CandidateSkillError("CANDIDATE_SKILL_DOCUMENT_SHA_INVALID")
        if not _SHA_RE.fullmatch(self.provenance_sha256):
            raise CandidateSkillError("CANDIDATE_SKILL_PROVENANCE_SHA_INVALID")
...
class _SkillOnlyReflectionRunner:
    """Filter the generic reflection output before candidate construction/staging."""
    ...
    def run(self, packet: Any) -> ReflectionResult:
        ...
        if _preview_reflection_result(result):
            return result
        # Fail closed without leaving a claimed replay receipt stranded. The parent
        # coordinator will complete this as NO_LEARNING_VALUE and stage nothing.
...
class _CandidateSkillStagingGateway:
    """Guard one exact candidate before delegating to the canonical stage-only gateway."""

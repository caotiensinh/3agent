...
class DailyReportAgent(BaseAgent):
    ...
    def _snapshots(self, ev: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for a in ev["activities"]:
            if a.get("task_id"): acts[str(a["task_id"])].append(a)
        for a in ev["artifacts"]:
            if a.get("task_id"): arts[str(a["task_id"])].append(a)
        result = []
        for t in ev["tasks"]:
            tid = str(t["task_id"])
            try: status = TaskStatus(str(t["status"]))
            except ValueError: status = TaskStatus.WAITING_HUMAN
            ...
            result.append({
                "task_id": tid, "title": str(t["title"]), "request": str(t["request"]), "status": status.value,
                "activity_count": len(ta), "artifact_count": len(tf),
                ...
                "evidence_ids": self._unique(eids), "blocker_evidence_ids": self._unique(blocker_ids),
                "blocker_details": self._unique(blocker_details), "suggested_next_action": NEXT[status],
            })
        return result
    ...
    def _deterministic(self, snaps: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
        ...
        return {"summary_points": work[:3], "work_items": work, "achievements": achievements,
                "blockers": blockers, "tomorrow_plan": tomorrow, "manager_attention": blockers[:3]}

    def _normalize(self, value: Any, valid_eids: set[str], valid_tids: set[str], require_tid: bool) -> tuple[list[dict[str, Any]], list[str]]:
        accepted, rejected = [], []
        if not isinstance(value, list): return accepted, rejected
        for raw in value:
            if not isinstance(raw, dict): rejected.append(str(raw)); continue
            text = str(raw.get("text", "")).strip()
            if not text: continue
            ids = raw.get("evidence_ids", [])
            ids = [x for x in ids if isinstance(x, str) and x in valid_eids] if isinstance(ids, list) else []
            tid = raw.get("task_id")
            if tid is not None and str(tid) not in valid_tids: rejected.append(text); continue
            if require_tid and not tid: rejected.append(text); continue
            if not ids: rejected.append(text); continue
            item = {"text": text, "evidence_ids": self._unique(ids)}
            if tid: item["task_id"] = str(tid)
...
Each item: {text, evidence_ids}; task-specific items also: task_id.\n\nEVIDENCE:\n""" + json.dumps(
        ...
        valid_eids, valid_tids = set(ev["evidence_index"]), {str(t["task_id"]) for t in ev["tasks"]}
        sections, rejected = {}, []
        ...
        for key in ("summary_points", "work_items", "achievements", "blockers", "tomorrow_plan", "manager_attention"):
            ...
            if not accepted and fallback[key]: rejected.append(f"{key}: deterministic evidence fallback used")
    ...
    def _markdown(self, date: str, status: str, sections: dict[str, Any], ev: dict[str, Any], rejected: list[str]) -> str:
        ...
        defs = [("## 1. 本日の要約", "summary_points"), ("## 2. 本日の業務", "work_items"),
                ("## 3. 成果・進捗", "achievements"), ("## 4. 課題・懸念事項", "blockers"),
                ("## 5. 明日の予定", "tomorrow_plan"), ("## 6. 上司確認事項", "manager_attention")]
        for heading, key in defs:
            ...
            if not sections.get(key): lines.append("- なし")
            lines.append("")
        if rejected:
            lines += ["## 7. AI出力検証", "以下の記述は有効なEvidence参照を持たないため除外または置換しました。"]
            lines += [f"- {x}" for x in rejected] + [""]
        lines.append("## Evidence")
        for t in ev["tasks"]: lines.append(f"- **[{t['evidence_id']}] Task** `{t['task_id']}` — {t['title']} (status: `{t['status']}`)")
        for a in ev["activities"]:
            details = str(a.get("details", "")).replace("\n", " ").strip()
            lines.append(f"- **[{a['evidence_id']}] Activity** {a['timestamp']} — `{a.get('task_id') or '-'}` / `{a['agent_id']}` / `{a['action']}` / `{a['status']}`" + (f" — {details[:240]}" if details else ""))
        for f in ev["artifacts"]: lines.append(f"- **[{f['evidence_id']}] Artifact** `{f.get('task_id') or '-'}` — `{f['artifact_type']}` / `{f['path']}`")
        if not ev["evidence_index"]: lines.append("- 対象日のEvidenceはありません。")
        return "\n".join(lines)
    ...
    def run(self, date: str, store: TaskStore, artifacts: ArtifactManager, live: bool = False):
        ...
        if live and ev["evidence_index"]:
            try: sections, rejected = self._live(date, ev, snaps); status = "model_generated_evidence_validated"
            except Exception as exc:
                sections = self._deterministic(snaps); status = "deterministic_fallback_after_model_error"
                rejected = [f"Live synthesis failed; deterministic fallback used: {type(exc).__name__}: {exc}"]
        else:
            sections = self._deterministic(snaps); status = "deterministic_from_evidence" if ev["evidence_index"] else "no_activity"
        payload = {"schema_version": 2, "date": date, "agent_id": self.agent_id, "status": status,
                   "activity_count": len(ev["activities"]), "evidence_digest": ev["evidence_digest"],
                   "source_counts": {k: len(ev[k]) for k in ("tasks", "activities", "artifacts")},
                   "task_snapshots": snaps, "sections": sections, "rejected_model_items": rejected,
                   "evidence": {k: ev[k] for k in ("tasks", "activities", "artifacts")}, "generated_at": datetime.now(TZ).isoformat()}

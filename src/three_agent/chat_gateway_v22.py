...
class ContinuitySecurityAwareProjectChatService(_v21.SecurityAwareProjectChatService):
    ...
    def _direct_prompt(self, job: Any, upload_ids: list[str]) -> str:
        ...
        # Preserve V21's bounded Security Analyst awareness after adding the
        # conversation/document context. This data is read-only and grants no
        # network, credential, packet-capture, remediation, or mutation authority.
        sections += ["", _v21._bounded_security_context(str(job.message or ""))]
        return "\n".join(sections)
...
# V21 remains the rollback/security boundary. V22 only replaces the final
# composed chat service, knowledge parser and UI document consumed by v17.main().

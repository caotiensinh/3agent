...
class DomainCompletionCapabilityIntegrationTests(unittest.TestCase):
    ...
    def test_explicit_grant_authorizes_only_exact_read_resource(self) -> None:
    ...
    def test_wrong_resource_kind_is_rejected(self) -> None:
        for tool_id, _, resource_ref in DOMAIN_COMPLETION_CASES:
            with self.subTest(tool_id=tool_id):
                contract = TaskContractCompiler().compile(
                    task_id=f"TASK-DOMAIN-COMPLETION-KIND-{tool_id}",
                    task_type="analysis",
                    sensitivity="internal",
                    allowed_tools=(tool_id,),
                )
                ...
                with self.assertRaisesRegex(
                    ...
                    authority.require(
                        tool_id,
                        resource_kind="diagnostic_resource",
                        resource_ref=resource_ref,
                        effect="read",
                    )

    def test_wrong_resource_ref_is_rejected(self) -> None:
        for tool_id, resource_kind, _ in DOMAIN_COMPLETION_CASES:
            with self.subTest(tool_id=tool_id):
                contract = TaskContractCompiler().compile(
                    task_id=f"TASK-DOMAIN-COMPLETION-REF-{tool_id}",
                    task_type="analysis",
                    sensitivity="internal",
                    allowed_tools=(tool_id,),
                )

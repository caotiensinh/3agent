...
def test_clean_bound_snapshot_has_no_findings() -> None:
    triage = DeterministicDhcpIpTriage([_asset("pc-01", "192.0.2.101")])
    assert triage.triage([_obs()]) == ()


def test_duplicate_ip_claim_requires_same_snapshot_and_distinct_clients() -> None:
    ...
    assert duplicate[0].snapshot_id == "snap-001"
    assert duplicate[0].asset_refs == ("asset:pc-01", "asset:pc-02")
    assert duplicate[0].authority == "advisory"
    assert duplicate[0].network_executed is False
    assert duplicate[0].discovery_performed is False
    assert duplicate[0].remediation_executed is False

    rendered = json.dumps(duplicate[0].public_dict(), sort_keys=True)
    assert "192.0.2.55" not in rendered
    assert "entity:ip:sha256:" in rendered


def test_lease_and_address_state_findings_are_deterministic() -> None:
    triage = DeterministicDhcpIpTriage(
        [
            _asset("pc-01", "192.0.2.101"),
            _asset("pc-02", "192.0.2.102"),
...
def test_observation_requires_enabled_approved_asset() -> None:
    triage = DeterministicDhcpIpTriage(
        [
            _asset("pc-01", "192.0.2.101"),
...
def test_conflicting_logical_snapshot_observation_fails_closed() -> None:
    triage = DeterministicDhcpIpTriage([_asset("pc-01", "192.0.2.101")])
    client = "same-client"

    with pytest.raises(MonitoringContractError, match="conflicting snapshot content"):
        triage.triage(
            [
...
def test_missing_lease_cannot_smuggle_address_metadata() -> None:
    with pytest.raises(MonitoringContractError, match="must not carry address metadata"):
        _obs(state="missing", assigned="192.0.2.10", subnet=None, gateway=None).validate()


def test_observation_iterator_stops_at_hard_bound() -> None:
    triage = DeterministicDhcpIpTriage([_asset("pc-01", "192.0.2.101")])
    observation = _obs()
    ...
    with pytest.raises(MonitoringContractError, match="observation bound exceeded"):

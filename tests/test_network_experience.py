...
def test_case_is_compact_evidence_not_raw_log_storage():
    case = ExperienceCase.from_dict(base_case())
    assert len(case.evidence) == 2
    assert case.confirmed_cause is None
    assert case.fingerprint().startswith("sha256:")


def test_confirmed_cause_requires_ground_truth_or_operator_verification():
    payload = base_case()
    payload["confirmed_cause"] = "physical link degradation"
    payload["cause_basis"] = "unknown"
    with pytest.raises(NetworkExperienceError):
        ExperienceCase.from_dict(payload)


def test_unverified_remediation_is_forbidden():
    payload = base_case()
    payload["remediation"] = [
        {
...
def test_verified_remediation_must_reference_case_evidence():
    payload = base_case()
    payload["remediation"] = [
        {
            "action": "replace cable",
...
def test_evidence_pattern_requires_multiple_independent_cases():
    payload = {
...
def test_candidate_skill_is_advisory_and_never_auto_promoted():
    payload = {
    ...
    assert skill.auto_promotable is False


def test_candidate_skill_cannot_self_approve():
    payload = {
        "schema_version": "workspace-network-skill-candidate/v1",
        "name": "diagnose-link-degradation",

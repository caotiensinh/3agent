...
class IcmpCollectorCrossPlatformTests(unittest.TestCase):
    def test_linux_ping_argv_is_bounded_and_native(self) -> None:
        collector = IcmpCollector(_policy_engine(), platform_name="posix")
        self.assertEqual(
            collector._argv("192.0.2.10"),
            ["ping", "-n", "-c", "1", "-W", "2", "192.0.2.10"],
        )

    def test_windows_ping_argv_is_bounded_and_native(self) -> None:
        collector = IcmpCollector(_policy_engine(), platform_name="nt")
        self.assertEqual(
            collector._argv("192.0.2.10"),
            ["ping", "-n", "1", "-w", "2000", "192.0.2.10"],
        )

    def test_rtt_parser_supports_linux_and_windows_reply_lines(self) -> None:
        cases = (
        ...
        for stdout, expected in cases:
            with self.subTest(stdout=stdout):
                self.assertEqual(IcmpCollector._parse_rtt_ms(stdout), expected)

    def test_rtt_parser_does_not_invent_reply_rtt_from_summary(self) -> None:
        for stdout in (
            ...
            with self.subTest(stdout=stdout):
                self.assertIsNone(IcmpCollector._parse_rtt_ms(stdout))

    def test_collect_uses_shell_false_and_windows_native_argv(self) -> None:
        ...
        self.assertEqual(captured["argv"], ["ping", "-n", "1", "-w", "2000", "192.0.2.10"])
        ...
        self.assertEqual(
            [(item.metric, item.status, item.value) for item in result.observations],
            [("icmp_reachable", "ok", True), ("icmp_rtt_ms", "ok", 1.0)],
        )

    def test_collect_unreachable_does_not_emit_rtt_without_reply_line(self) -> None:
        def executor(argv: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
            return subprocess.CompletedProcess(
                argv,
                1,
        ...
        self.assertEqual(result.failure_code, "ICMP_UNREACHABLE")
        self.assertEqual(len(result.observations), 1)
        self.assertEqual(result.observations[0].metric, "icmp_reachable")
        self.assertEqual(result.observations[0].status, "unreachable")
        self.assertIs(result.observations[0].value, False)

    def test_collect_missing_ping_binary_fails_closed(self) -> None:
        def executor(argv: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        self.assertEqual(result.failure_code, "PING_BINARY_UNAVAILABLE")
        self.assertEqual(len(result.observations), 1)
        self.assertEqual(result.observations[0].status, "unsupported")

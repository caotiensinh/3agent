...
class FakeBridge:
    ...
    def begin(self, task_id, *, contract=None):
        ...
        return SimpleNamespace(
            contract_sha256=self.store.task_contract_record(task_id)["contract_sha256"],
...
class WorkflowStateMachineV3Tests(unittest.TestCase):
    ...
    def test_prepare_binds_24h_bounded_contract_without_execution(self):
        ...
        state_text = self.controller._state_path(prepared["task_id"]).read_text(encoding="utf-8")
    ...
    def test_passed_validation_pauses_then_approve_resumes_exact_node_once(self):
        ...
        self.assertEqual(self.store.get_task(prepared["task_id"]).status, TaskStatus.WAITING_HUMAN)

        completed = self.controller.decide_checkpoint(
            prepared["task_id"],
    ...
    def test_reject_selects_rejected_branch_without_presentation(self):
        prepared = self._prepare()
        paused = self._start(prepared)
    ...
    def test_failed_validation_chooses_failed_branch_and_never_pauses(self):
    ...
    def test_wrong_checkpoint_fingerprint_and_confirmation_are_fail_closed(self):
        prepared = self._prepare()
        paused = self._start(prepared)
    ...
    def test_arbitrary_conditions_join_schedule_and_non_low_risk_are_rejected(self):
    ...
    def test_status_response_is_metadata_only(self):
        ...
        self.assertEqual(result["checkpoint"]["fingerprint"], paused["checkpoint"]["fingerprint"])

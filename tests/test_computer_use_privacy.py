...
def restricted_runtime():
    ...
    workflow = {
        ...
        "nodes": [
            ...
            {"id": "observe", "label": "Observe", "kind": "agent", "action": "research", "depends_on": ["start"], "condition": "", "approval_required": False},
            {"id": "done", "label": "Return", "kind": "output", "action": "output", "depends_on": ["observe"], "condition": "", "approval_required": False},
        ],
    ...
    plan = ExecutionPlanBuilder.build(
        ...
        node_bindings={
            "observe": ExecutionNodeBinding(
                allowed_sources=("evidence",),
                allowed_tools=("browser.dom.observe",),
                resource_budget=TaskResourceBudget(wall_time_s=10, model_tokens=100, tool_calls=1),
            ),
        },
    )
...
class ComputerUsePrivacyTests(unittest.TestCase):
        ...
        sanitized = sanitize_computer_content(
            {
                ...
                "nested": {"type": "password", "value": "hunter2", "textContent": "hunter2"},
    ...
    def test_inline_credentials_and_private_keys_are_redacted(self):
        private_key = "-----BEGIN PRIVATE KEY-----\nABCDEF123456\n-----END PRIVATE KEY-----"
        sanitized = sanitize_computer_content(
            {
                "text": "Authorization: Bearer abcdefghijkl password=hunter2 cookie=sid-secret",
                "terminal": private_key,
            }
        )
        retained = str(sanitized)
        for secret in ("abcdefghijkl", "hunter2", "sid-secret", "ABCDEF123456"):
            self.assertNotIn(secret, retained)
        self.assertIn(REDACTED, retained)

    def test_url_userinfo_query_and_fragment_are_not_retained(self):
        sanitized = sanitize_computer_content({"url": "https://user:pass@example.test/path?token=secret#private"})
    ...
    def test_continuous_capture_payload_is_rejected(self):
        with self.assertRaisesRegex(ComputerPrivacyError, "COMPUTER_CONTINUOUS_CAPTURE_RETENTION_FORBIDDEN"):
    ...
    def test_evidence_adapter_rejects_continuous_capture_result(self):
        authority, plan = restricted_runtime()
        pre, post = observations(plan.task_id)
        with self.assertRaisesRegex(ComputerEvidenceError, "COMPUTER_EVIDENCE_PRIVACY_REJECTED"):

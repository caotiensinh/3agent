...
# Core/Public may reach only the constrained Unix-socket broker; nftables still denies direct egress.
...
log "Confidential Core UID=${CORE_UID}: localhost Ollama + constrained broker IPC; no direct Internet/LAN egress."
log "Public Research UID=${PUBLIC_UID}: separate DB/data, localhost Ollama only, constrained broker IPC, no confidential-data permission."
